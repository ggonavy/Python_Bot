...
PAIR = 'BTCUSDT'
DECIMALS = 6  # Binance prefers 6+ decimals for BTC amounts

# === Load API keys from environment variables (Render compatible) ===
...
# === State ===
high_price = 0.0  # tracks most recent high
buys = []         # tracks active buys with metadata
...
print("🧠 SlateBot_Binance is live and scanning...")
...
while True:
    try:
        ...
        if current_price > high_price:
            ...
            print(f"[HIGH] New high tracked: ${high_price:.2f}")

        # Check balances
        fiat_balance, btc_balance = get_balances()
        total_fiat_start = fiat_balance + sum([b['usdt_used'] for b in buys if not b['sold']])
        ...
        for entry in BUY_LADDER:
            ...
            if current_price <= dip_price:
                # Check if this ladder was already filled
                if not any(abs(b['drop_pct'] - entry['drop_pct']) < 0.01 for b in buys):
                    ...
                    if usdt_to_spend <= fiat_balance:
                        ...
                        print(f"[BUY] –{entry['drop_pct']}% drop: Bought {quantity} BTC @ ${price:.2f}. Sell target: ${high_price * (1 + entry['sell_trigger_pct'] / 100):.2f}")
        ...
        for b in buys:
            if not b['sold'] and current_price >= b['sell_trigger']:
                ...
                b['sold'] = True
                print(f"[SELL] +{BUY_LADDER[buys.index(b)]['sell_trigger_pct']}% target hit. Sold {b['btc_bought']} BTC @ ${current_price:.2f}")

        # === Reset Cycle ===
        if all(b['sold'] for b in buys) and buys:
            print("[RESET] All positions sold. Resetting state.")
            buys.clear()
            high_price = 0.0
