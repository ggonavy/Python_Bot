...
# === STRATEGY PARAMETERS ===
BUY_LADDER = [(47, 0.10), (42, 0.20), (37, 0.30), (32, 1.00)]  # Full buy at 32
...
def place_buy_order(pct, fiat):
    usd_amount = fiat * pct
    print(f"💰 Placing Buy: {pct*100:.0f}% of ${fiat:.2f} = ${usd_amount:.2f}")
...
def place_sell_order(pct, btc):
    btc_amount = btc * pct
    print(f"🔻 Placing Sell: {pct*100:.0f}% of {btc:.6f} BTC = {btc_amount:.6f} BTC")
    # Uncomment to execute real order:
...
def main():
    ...
    while True:
        ...
        time.sleep(1)  # ✅ 1-second interval — NO SLEEP MODE achieved
